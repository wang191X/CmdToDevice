...
int CWsnmp::snmpWalk()
{
	...
	UdpAddress address( ip.c_str());      // make a SNMP++ Generic address
	if ( !address.valid()) {           // check validity of address
		//cout << "Invalid Address or DNS Name, " << argv[1] << "\n";
		return 1;
	}

	//Oid oid("1.3.6.1.2.1.31.1.1.1.1");
	...
	while (( status = snmp.get_bulk( pdu,*target,0,BULK_MAX))== SNMP_CLASS_SUCCESS) {
		...
		for ( int z=0;z<pdu.get_vb_count(); z++) {
			...
			if ( vb.get_syntax() != sNMP_SYNTAX_ENDOFMIBVIEW) {
				...
				string tmpStr(vb.get_printable_oid());
				int index=tmpStr.find_last_of(".");
				tmpStr=tmpStr.substr(index+1,tmpStr.length()-index-1);

				resultMap.insert( Int_Pair( tmpStr, vb.get_printable_value() ) );
		 }
			else {
...
int CWsnmp::determine_vb(SmiUINT32 val, Vb &vb)
{
	char buffer[255];
...
int CWsnmp::snmpSet()
{
	...
	Oid oid(oidStr.c_str());      // default is sysName
...
#endif
		if ( determine_vb(sNMP_SYNTAX_INT, vb)) {
			// do the Set
			Pdu setpdu;
            ...
            string tmpStr(snmp.error_msg(status));
